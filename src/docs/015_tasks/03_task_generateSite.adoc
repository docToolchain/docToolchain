...
One setting has no sensible convention: the address your site is published under.
No build can guess it, so `generateSite` asks for it the first time a theme is used
in a project and stores the answer as `microsite.host`.
...
The sitemap is built from it. A sitemap tells crawlers which addresses exist, so a
placeholder here would publish a promise the site cannot keep. When `host` is still
unset, `generateSite` writes an empty sitemap and warns, rather than a full one that
leads nowhere.
