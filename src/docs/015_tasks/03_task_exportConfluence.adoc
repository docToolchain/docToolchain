...
[TIP]
====
If you only need a subtree (not a full space), don't have Space Administrator rights, or can't get an XML export, use xref:03_task_exportConfluenceFromApi.adoc[exportConfluenceFromApi] instead.
It fetches pages and attachments over the REST API and produces the same AsciiDoc output shape.
====
...
==== partial export (subtree)

By default the task converts every current page found in the space export.
You can also restrict the export to a single page and all of its descendants.

NOTE: You still need a full space XML export from Confluence — Confluence does not produce per-subtree exports, so the filter is applied during conversion.

Configure the root page in `docToolchainConfig.groovy` using either the page id or the exact page title:
...
Or pass the parameters on the command line:

[source, bash]
----
./dtcw exportConfluence \
    -Pconfluence.export.srcDir=sample_data \
    -Pconfluence.export.destDir=src/docs \
    -Pconfluence.export.rootPageId=123456789
----

If both `rootPageId` and `rootPageTitle` are provided, `rootPageId` wins.
`rootPageTitle` must match exactly one page in the export; otherwise the task fails with an error.
...
:param_source_file: scripts/confluenceToAsciiDoc.groovy
