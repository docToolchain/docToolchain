...
//What do I have to do? What if I am on Windows bit my colleagues are on Linux? 
...
//Do we need this information to get started the first time? Otherwise transfer it to the end of this file (or in another one altogether)
...
// I have absolutely no idea what the next paragraph means and if the source-part belongs to this bullet point ;(
** build-script dependencies are fetched from a repository referenced by the property `mavenRepository`. By default the value `https://plugins.gradle.org/m2/` is used. When a repository requiring credentials is used the properties `mavenUsername` and `mavenPassword` can be set as well.
