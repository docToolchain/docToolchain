...
We use https://semver.org/[semantic versioning] and https://keepachangelog.com/[keep a changelog]. Both on a best effort base.

A release consists of five parts:
...
Now it is time to create a blog post which announces the new release.
The sdkman announcement will reference it.

=== docToolchain-Wrapper (dtcw)

everything went fine? Good! Now let's setup the wrapper to use the new version.
