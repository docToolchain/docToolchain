...
microsite.with {
    ...
    // These let you re-brand the microsite without copying the whole theme.
    // Every value is optional: leave it empty/unset to keep the theme default.
    //
    // logo: path to your logo, relative to the site's asset root. Put the file
    // into your site theme's assets/images folder (see 'copyThemes jBakeTheme').
    logo = 'images/doctoolchain-logo-blue.png'
    //
    // colors: any CSS color value (hex, rgb(), hsl(), ...). They override the
    // theme's CSS custom properties for both the light and the landing page.
    colorPrimary     = ''   // accent / brand color (links, buttons, highlights)
    colorPrimaryDark = ''   // darker shade used in gradients
    colorLink        = ''   // body link color
    colorInk         = ''   // main text color
    colorBackground  = ''   // page background
    colorCard        = ''   // card / panel background
    //
    // fonts: a CSS font-family value, e.g. "'Roboto', sans-serif".
    // The bundled fonts are Space Grotesk (headings) and IBM Plex (body/mono).
